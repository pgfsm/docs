---
title: "PRD-002 — Scaffold async operation logic (actors / `invoke` objects)"
description: "Product requirements for scaffolding base actor / invoke-object stub code from a compiled fsm.json."
---
...
[`DEVELOPER.md`](/docs/guides/developer-guide), the same section that covers sync
...
- Validating/loading the filled-in stubs (covered by
  [PRD-004](/docs/packages/fsm-compiler-ts/prd/prd-004-validate-async-operation-logic)).
...
- Deduplicates by language + sanitized `src` (`${lang}/${src}`) — actors
  ...
  currently collapse into the same file (see [TODO.md](/docs/packages/fsm-compiler-ts/todo/todo), PRD-002
  dedup-key gap).
