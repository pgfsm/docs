---
title: "ADR-002: Node.js + npm, with a Pinned `supabase` CLI Version, for `database-src`"
description: "Decision to run database-src on Node.js + npm with a pinned supabase CLI version rather than Deno."
---
