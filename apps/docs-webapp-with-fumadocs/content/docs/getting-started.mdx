...
description: "Quick-start guide for building an FSM app against the published @pgfsm/* packages: design, scaffold, run the workers, and control the cluster."
---
...
> repo, see [DEVELOPER.md](/docs/guides/developer-guide) instead — same lifecycle,
...
[the package's own README](/docs/packages/fsm-compiler-ts) for the full
...
[the package's own README](/docs/packages/fsm-compiler-ts) for details and
examples.
...
its actors — see
[the package's own README](/docs/packages/fsm-core-async-op-worker) for
...
[the package's own README](/docs/packages/fsm-sync-worker-ts) for the full
...
See [the sync-worker package's README](/docs/packages/fsm-sync-worker-ts)
(`fsmctl`) and
[the async-worker package's README](/docs/packages/fsm-core-async-op-worker)
...
- Compiler CLI (`@pgfsm/compiler`) —
  [`packages/fsm-compiler-ts/README.md`](/docs/packages/fsm-compiler-ts)
- Sync worker CLI (`@pgfsm/sync-worker`) —
  [`packages/fsm-sync-worker-ts/README.md`](/docs/packages/fsm-sync-worker-ts)
- Async-operation worker CLI (`@pgfsm/async-worker`) —
  [`packages/fsm-core-async-op-worker/README.md`](/docs/packages/fsm-core-async-op-worker)
...
- Developing or debugging this framework itself, in this repo —
  [`DEVELOPER.md`](/docs/guides/developer-guide)
